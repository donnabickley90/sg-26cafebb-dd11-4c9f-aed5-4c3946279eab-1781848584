...
export default function WeeklyReset() {
  ...
  const loadTasks = () => {
    const weeklyTasksDue = getWeeklyTasksDueThisWeek();
    
    // Group by room
    const grouped = new Map<string, CleaningTask[]>();
    PREDEFINED_ROOMS.forEach(room => {
      const roomTasks = weeklyTasksDue.filter(t => t.room === room);
    ...
    // Calculate totals
    const allTasks = getCleaningTasks();
    const weeklyTasks = allTasks.filter(t => t.frequency === "weekly");
    const completed = weeklyTasks.length - weeklyTasksDue.length;
    
    setTotalCount(weeklyTasks.length);
    setCompletedCount(completed);
    
    // Check if all done
    if (weeklyTasks.length > 0 && weeklyTasksDue.length === 0) {
      setShowSuccess(true);
    }
  };
  ...
  const isCompletedThisWeek = (task: CleaningTask): boolean => {
          ...
          <ThemedCardContent className="pt-6 text-center space-y-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-muted-foreground">
                All {totalCount} weekly tasks are done. Excellent work!
              </p>
            </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-3 flex-1">
            ...
            <div>
              <h1 className="font-heading text-3xl font-bold text-foreground">Weekly Reset</h1>
              <p className="text-sm text-muted-foreground">Check off this week's tasks</p>
            </div>
          </div>
        </div>
        ...
        <ThemedCard variant="glow">
          <ThemedCardContent className="pt-6 text-center py-12">
            ...
            <p className="text-muted-foreground mb-4">
              No weekly tasks due this week. Great work!
            </p>
            <Link href="/cleaning">
          ...
          {Array.from(tasksByRoom.entries()).map(([room, tasks]) => (
            <ThemedCard key={room} variant="glow">
              ...
              <ThemedCardContent>
                <div className="space-y-2">
                  {tasks.map(task => {
                    const isComplete = isCompletedThisWeek(task);
